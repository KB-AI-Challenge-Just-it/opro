...
| `SbizCollector` | 소진공 상가업소 상권정보 API | `market_snapshot` | 위와 동일 | **⚠️ 사실상 아니오 — 코드 불일치로 데모 계정 1개만 반영, 그 외 전원 null** |
...
## 4. SbizCollector — 상권 데이터 (⚠️ 실사용자 경로에서 사실상 끊겨 있음)
...
**결론**: `SbizCollector`는 API 연동·저장 로직 자체는 정상 동작하지만, **온보딩에서 실제로 발급되는 코드값과 수집기가 채우는 코드값이 애초에 서로 다른 체계라 절대 만나지 않는다.** `demo` 계정으로 로그인했을 때만 `market_context`가 실제로 채워지고, 그 외 모든 신규 가입자에게는 상권 데이터가 조용히 생략된다(에러 없이 `null` → 프롬프트가 "없으면 언급 안 함"으로 우아하게 처리하므로 사용자에게 장애로 보이지는 않는다).

---

## 5. 왜 이런 구조가 됐는가 (정당한 스코프 축소, 숨겨진 버그 아님)
...
- 4주 해커톤 범위에서는 이 매핑 테이블을 만드는 대신, **데모 시나리오 1건(강남 카페)만 좌표를 하드코딩**하고 코드에 "프로덕션 확장 시 매핑 테이블 필요"라는 TODO를 명시적으로 남기는 쪽을 선택했다.
- 즉 이건 **의도적으로 좁혀놓고 코드에 그 사실을 남긴 스코프 축소**다 — PRD 자체가 "MVP 범위는 의도적으로 좁게 잡는다"(§2)고 원칙을 밝혔던 것과 일관된다. 대회 발표에서 "왜 이렇게 했는지"를 정직하게 설명할 수 있는 지점이다.
...
flowchart LR
    subgraph 수집["L1 · 외부 데이터 수집 (매일 06:00 / 기동 시 1회 / 수동 /api/agent/collect)"]
        ...
        SBIZ["SbizCollector<br/>소진공 상가업소 API<br/>반경 500m 경쟁강도<br/>⚠️ 강남 카페 1건만 하드코딩"]
    end

    subgraph 저장["PostgreSQL"]
        PA[("policy_announcement<br/>수천 건")]
        EI[("econ_indicator<br/>전국 3개 지표")]
        MS[("market_snapshot<br/>region_code='A1001' 단 1행")]
    end
    ...
    subgraph 소비["실제 소비 지점"]
        ...
        DIAG_MKT["콜1 진단 / L3 적합성설명<br/>market_context<br/>⚠️ demo 계정 1개만 non-null"]
    end
    ...
    PA --> MATCH
    ECOS --> EI --> DIAG_ECON
    SBIZ --> MS -.->|"온보딩이 채우는 코드('11440'/'I56194')와<br/>수집기가 채우는 코드('A1001'/'카페·디저트')가<br/>애초에 다른 체계라 불일치"| DIAG_MKT
```
...
| 수집기 | 코드 품질(수집·저장 로직 자체) | 실제 도달률 |
|---|---|---|
| Bizinfo | 정상, 방어적 에러 처리 잘 돼 있음 | 100% — 전체 매칭 파이프라인의 근간 |
| Ecos | 정상, 舊 트리거용 명명 규칙만 죽은 계약으로 남음 | 100%(API 키 설정 시) — 단, 개인화 아닌 공통값 |
| Sbiz | 정상, 다만 타겟이 1건 하드코딩 | 사실상 0%(데모 계정 1개 제외) — 온보딩 목업 코드와 불일치 |
