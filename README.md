...
사장님이 직접 검색하는 구간은 온보딩 한 번뿐입니다. 그 이후(추가 질문 답변부터 알림까지)는 전부 에이전트가 먼저 움직입니다.
...
| 작업 | 모델 등급 | 선택 이유 |
| --- | --- | --- |
| 진단·추가 질문 | Opus | 이후 전체 파이프라인(재질문·검색·리포트)에 영향을 주는 유일한 시작점 |
| 적합성 판단·리포트·초안 | Sonnet | 판단과 설명 품질이 필요한 추론 작업 |
| 검색 쿼리 변환 | Haiku | 판단이 아닌 정형화된 경량 변환 작업 |
