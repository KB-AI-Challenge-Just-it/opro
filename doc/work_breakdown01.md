...
| 메서드 · 경로 | 상태 | 설명 |
| --- | --- | --- |
| `POST /api/onboarding` | 🔶 이슈 #29 확장 중 | 질문지 제출 → 프로필 생성 → **저장 직후 동기로 프로필 기반 매칭 트리거 실행**(신규, 이슈 #29) |
...
| `POST /api/agent/check/{profileId}` | 🔶 이슈 #29 재정의 중 | 데모용 즉시 실행 — **지표 임계값 평가(구) → 프로필 기반 매칭 트리거(신)로 내부 동작 교체**, 경로·응답 형태는 유지 |
...
| `POST /analysis` | 🔶 이슈 #29 재정의 중 | L3 적합성 설명 (Sonnet) — **역할 변경**: "지표 원인분석"이 아니라 "이 공고가 왜 프로필에 맞는지" 설명. **매칭 있을 때만 호출** | `{profile, matches: [{pblanc_id, title, evidence}...], market_context?}` → `{fit_text: str}` |
...
| S4 | TRIGGER · 트리거 엔진 | P1 | S3 | 🔶 이슈 #29 개편 중 | **지표 임계값 트리거(`TriggerEngine.latestMetric()`) 폐기 → `ProfileMatchTrigger`(프로필 기반 공고 매칭 트리거)로 교체.** 온보딩 직후 1회성 매칭 + `ScheduledJobs.dailyRun()`의 신규 공고 재매칭. `profile_funding_alert` 신규 테이블로 dedup. 상세: 이슈 #29 |
...
| E3 | L3 · 적합성 설명 프롬프트 (구 원인분석) | P1 | — | 🔶 이슈 #29 재정의 중 | `cause_analysis.py`/`/analysis` 역할 전환: "지표 원인분석"→"매칭된 공고가 왜 프로필에 맞는지 설명". 입력이 `trigger_context`→`matches`로 변경. §2-2 계약 갱신 반영 |
