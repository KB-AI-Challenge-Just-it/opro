...
### 1. Chroma 컬렉션에 임베딩 함수 명시 (최우선 — 데모 품질 직결)

`vectorstore.py`의 `get_or_create_collection()`이 `embedding_function` 미지정 → Chroma 기본값(all-MiniLM-L6-v2, **영어 학습 모델**)으로 한국어 공고를 임베딩 중이다. 하이브리드의 벡터 축이 열화된 채 BM25가 검색을 떠받치고 RRF가 이를 가리는 상태 — PRD 차별점("RAG 근거 매칭")의 시맨틱 절반이 손상된다.

- 한국어를 지원하는 다국어 임베딩을 명시한다 (예: `BAAI/bge-m3`, `intfloat/multilingual-e5-base` 계열 — 설치 크기·속도 검토 후 택1)
- 임베딩 함수 변경 시 기존 컬렉션과 차원이 달라지므로 **컬렉션 재생성 + `/index/rebuild` 필수**
...
- 응답 JSON 스키마는 `doc/work_breakdown01.md` §2-2 계약 그대로 — 필드 추가/변경은 문서 먼저
- 프롬프트·검색 튜닝은 골든 I/O 예시(데모 시나리오 쿼리 → 기대 top-5)를 기준으로 회귀 확인
- 튜닝 평가 시 `e2e-verify` 시드 공고 3건(DEMO-000x)이 최소 평가셋이다
