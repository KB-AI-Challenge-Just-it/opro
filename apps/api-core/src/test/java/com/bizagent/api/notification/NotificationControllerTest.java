...
class NotificationControllerTest {
    ...
    /**
     * POST /api/notifications/kakao-test — 수동 QA용 카카오 재발송.
     * report/notification/notification_delivery 세 테이블을 흉내내는 fake JdbcTemplate으로 검증한다.
     * queryForObject(SELECT profile_id...)/INSERT ... RETURNING id/queryForList(delivery) 세 종류
     * 호출을 모두 커버해야 하므로 by-report 테스트의 FakeJdbc보다 범용적인 fake를 새로 둔다.
     */
    private static final class FakeKakaoTestJdbc extends JdbcTemplate {
        Long reportOwnerProfileId; // report_id → profile_id (단일 report 가정)
        long nextNotificationId = 1;
        Long lastNotificationId;
        String deliveryStatusToReturn; // null이면 delivery row 없음(NOT_CONNECTED)
        boolean sendCalled;

        @Override
        @SuppressWarnings("unchecked")
        public <T> T queryForObject(String sql, Class<T> requiredType, Object... args) {
            if (sql.contains("SELECT profile_id FROM report")) {
                if (reportOwnerProfileId == null) {
                    throw new org.springframework.dao.EmptyResultDataAccessException(1);
                }
                return (T) reportOwnerProfileId;
            }
            if (sql.contains("INSERT INTO notification")) {
                lastNotificationId = nextNotificationId++;
                return (T) lastNotificationId;
            }
        ...
        public <T> List<T> queryForList(String sql, Class<T> elementType, Object... args) {
            if (sql.contains("notification_delivery")) {
                return deliveryStatusToReturn == null ? List.of() : List.of((T) deliveryStatusToReturn);
            }
            throw new UnsupportedOperationException(sql);
        }
    }
    ...
    @Test
    void sendKakaoTest_returnsSent_whenSenderRecordsDelivery() {
        FakeKakaoTestJdbc jdbc = new FakeKakaoTestJdbc();
        jdbc.reportOwnerProfileId = OWNER_PROFILE;
        jdbc.deliveryStatusToReturn = "SENT";
        NotificationSender sender = mock(NotificationSender.class);

        var result = new NotificationController(mock(NotificationRepository.class), jdbc, sender)
                .sendKakaoTest(REPORT_ID, OWNER_PROFILE);

        verify(sender).send(OWNER_PROFILE, jdbc.lastNotificationId, REPORT_ID, "[테스트] 카카오 알림 발송 확인");
        assertThat(result).containsEntry("status", "SENT");
    }

    @Test
    void sendKakaoTest_returnsNotConnected_whenNoDeliveryRowRecorded() {
        // KakaoMemoSender는 kakao_token 미동의 시 delivery row 자체를 안 남긴다 — 그 신호를 구분해야 한다.
        FakeKakaoTestJdbc jdbc = new FakeKakaoTestJdbc();
        jdbc.reportOwnerProfileId = OWNER_PROFILE;
        jdbc.deliveryStatusToReturn = null;

        var result = new NotificationController(mock(NotificationRepository.class), jdbc, mock(NotificationSender.class))
                .sendKakaoTest(REPORT_ID, OWNER_PROFILE);

        assertThat(result).containsEntry("status", "NOT_CONNECTED");
    }

    @Test
    void sendKakaoTest_rejectsOtherUsersReport() {
        FakeKakaoTestJdbc jdbc = new FakeKakaoTestJdbc();
        jdbc.reportOwnerProfileId = OWNER_PROFILE;
        NotificationController controller =
                new NotificationController(mock(NotificationRepository.class), jdbc, mock(NotificationSender.class));

        org.junit.jupiter.api.Assertions.assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.sendKakaoTest(REPORT_ID, OTHER_PROFILE));
    }
}
