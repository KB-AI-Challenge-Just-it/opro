...
> ⚠️ **[2026-07-28 갱신] 이슈 #29는 이미 구현 완료됐고, 그 이후로도 한 번 더 바뀌었다 — 이 문서는 역사적 스냅샷으로 취급할 것.**
> 이 문서는 원래 2026-07-19에 이슈 [#29](https://github.com/KB-AI-Challenge-Just-it/opro/issues/29)의 **목표 구조**(구현 전)로 작성됐다. 그 목표는 커밋 `08ee23d`(2026-07-21)로 실제 구현됐다 — `TriggerEngine.java`/`threshold_rule` 임계값 트리거는 폐지됐고, 아래 §1~§2가 서술하는 "온보딩 → 프로필 기반 즉시 매칭" 구조가 그 자리를 대신했다.
> 다만 **2026-07-24 대화형 2-콜 컨설팅 도입**으로 온보딩 직후 흐름이 한 번 더 바뀌었다: 이 문서가 §1에서 서술하는 "온보딩 제출 → (동기) 즉시 매칭"은 더 이상 현재 동작이 아니며, 지금은 "온보딩 → 콜1 진단(Opus) → 재질문 → 콜2 전문화 매칭"으로 동작한다. 즉 이 문서는 **"임계값 트리거 폐지" 시점의 스냅샷**으로는 정확하지만 최신 상태는 아니다.
> **현재 코드 기준 최신 아키텍처는 `doc/2026-07-28/competition_submission_architecture_2026-07-28.md`를 참고할 것.**
...
**이슈 #29 개편 직후엔 이 이해가 맞았다.** (개편 전에는 "지표 변화가 방아쇠"였고 온보딩 입력은 프로필 저장에 그쳤지만, 이슈 #29로 그 지표 임계값 트리거를 폐지하고 아래 구조로 바꿨다.)
> ⚠️ 단, 아래 1번 항목의 "그 직후(동기) 즉시 매칭"은 2026-07-24 대화형 2-콜 컨설팅 도입으로 다시 바뀌었다 — 지금은 온보딩 제출 후 즉시 매칭하지 않고, 콜1 진단·재질문을 먼저 거친 뒤 콜2에서 매칭한다. 최신 흐름은 `doc/2026-07-28/competition_submission_architecture_2026-07-28.md` §4 참고.
...
## 5. 웹 상에서 전체 플로우 테스트하는 방법 (이슈 #29 시점 — 이후 대화형 컨설팅 도입으로 다시 바뀜)

> ⚠️ 아래는 이슈 #29가 막 구현됐던 시점(2026-07-21)의 테스트 절차이며, 2026-07-24 대화형 2-콜 컨설팅 도입 이후에는 3번 항목(온보딩 제출 직후 동기 즉시 매칭)이 더 이상 맞지 않는다. 최신 절차는 `doc/testing/local_test_guide.md`를 단일 소스로 참고할 것.

이슈 #29 구현 직후에는 `doc/testing/local_test_guide.md`의 STEP 2~7(수동 curl 트리거) 대신 아래처럼 테스트했다:

1. `.env` 준비, `docker compose up -d --build`는 동일.
2. 데모 공고 시드 + `/index/rebuild`는 동일.
3. **웹 UI에서 온보딩 질문지 제출** → 그 자리에서(동기) 프로필 기반 매칭이 실행되고, 매칭되면 곧바로 리포트가 생긴다 — 개편 전과 달리 **`curl /api/agent/check/1` 같은 별도 강제 발동이 필요 없어진다.** 이게 이슈 #29의 체감 변화 중 하나: "웹 UI만으로 전체 파이프라인 확인 가능."(※ 이후 컨설팅 도입으로 "제출 즉시 리포트"가 "제출 → 콜1 진단 → 재질문 → 콜2 매칭"으로 다시 바뀌었다.)
4. 신규 공고 재매칭(배치)은 여전히 스케줄러 기반이라, 데모 중 즉시 확인하려면 배치를 수동 트리거하는 엔드포인트가 필요할 수 있음.
