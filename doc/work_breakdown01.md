...
| `POST /matching` | ✅ 이슈 #61 확장 | L4 하이브리드 RAG — 내부 로직(쿼리변환→BM25∥벡터→RRF) 무변경. `cause_text`엔 Spring이 프로필로 조립한 요약 텍스트가 들어감(이슈 #29). **매칭 결과에 `target`·`support_field`·`summary`(truncate된 공고 원문) 추가**(이슈 #61) — L3가 제목만 보고 답하던 문제 해결용. `evidence`는 RRF/BM25 순위 노출에서 실제 매칭 근거(검색어-공고 키워드 일치 + 지원대상·지원분야)로 교체 | `{cause_text, profile_hint?, top_k?=5}` → `{matches: [{pblanc_id, title, apply_end, detail_url, target, support_field, summary, evidence: str, rrf_score, bm25_rank, vector_rank}]}` |
