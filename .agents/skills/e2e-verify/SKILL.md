...
PRD 성공 기준 "트리거 발동 → Codex 원인 분석 → 정책자금 매칭까지 자동 파이프라인 완주"를 재현 가능한 절차로 검증한다. 수집기가 스텁이어도 시드 데이터로 파이프라인 자체를 완주시킬 수 있다 — 이것이 이 스킬의 존재 이유다.
...
```bash
docker compose exec -T postgres psql -U bizagent -d bizagent < .Codex/skills/e2e-verify/scripts/seed_demo.sql
