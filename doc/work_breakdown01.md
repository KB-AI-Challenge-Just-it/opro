...
| `POST /matching` | 🔶 이슈 #67 진행 중 | L4 하이브리드 RAG. **매칭 이전 단계 하드 필터 신설**(이슈 #67) — `region`(기존에 존재했으나 미사용이던 컬럼)·업종(휴리스틱, `target` 텍스트 기반)이 불일치하는 후보는 BM25∥벡터→RRF 이후 top_k로 자르기 전에 제외한다. 즉 **`matches`에 담겨 나오는 건수 자체가 "지역·업종 하드 조건을 통과한" 건수** — Spring·리포트는 이 길이를 그대로 "적합 공고 수"로 써도 된다. **`profile_hint?: str` 필드를 구조화된 `profile?` 객체로 교체**(기존 `profile_hint`는 Spring이 실제로 채워 보낸 적이 없던 죽은 필드였음 — `AiEngineClient.java`가 `cause_text`만 보내고 있었음, 이슈 #67에서 같이 수정). `evidence`는 토큰 겹침 카운트 대신 지역/업종 일치 여부 + 리스크 필드(체납·연체) 경고 문구로 재구성 | `{cause_text, profile?: {region_sido, region_sigungu, industry, tax_delinquency: string, overdue_status: string}, top_k?=5}` (`tax_delinquency`/`overdue_status`는 `BusinessProfile`과 동일한 문자열 enum — `taxDelinquency`: `NONE`/`YES`/`UNKNOWN_CONFIRMED`/`UNKNOWN_UNCONFIRMED`, `overdueStatus`: `NONE`/`RESOLVED`/`CURRENT`/`UNKNOWN_...`. `NONE` 또는 누락이면 리스크 없음으로 취급) → `{matches: [{pblanc_id, title, apply_end, detail_url, target, support_field, summary, evidence: str, rrf_score, bm25_rank, vector_rank}]}` (응답 스키마 필드는 동일, `evidence` 문구 내용만 재설계) |
