...
> **분석 기준**: 구 `work_breakdown01.md`(2026-07-21 삭제, 당시 §2 API 계약 · §3 작업 패키지) · `doc/decisions/001-notification-channel-kakao.md`
...
**설명**
당시(구 work_breakdown01 §3)에서 확인된 미완료 코드 픽스 3건:
...
**의존**: 없음 — `AiEngineClient.java`와 ai-engine 라우터 스키마를 같은 커밋에서 갱신
...
(이후 이슈 #67에서 `evidence` 필드가 실제로 추가·재설계됨 — 이 항목은 완료됨)

**Acceptance Criteria**
- [ ] `/matching` 응답 스키마에 `evidence` 필드 추가
...
응답 스키마는 `NotificationController.java`를 준수한다:
```json
...
**설명**
당시(구 work_breakdown01 부록)의 오픈 결정. `01_schema.sql`이 `CREATE EXTENSION IF NOT EXISTS vector`를 실행하지만 벡터 컬럼을 쓰는 테이블이 없다. 임베딩은 전부 Chroma. README에는 "pgvector+Chroma 병기"로 표기되어 있어 정리가 필요하다.
...
| 항목 | 근거 |
|------|------|
| 인증/로그인 | 당시엔 profileId 기반 데모 페르소나로 대체(KB 인프라 위임 전제)였으나, 이후 평문 id/pw 회원가입·로그인이 실제로 도입됨(이슈 #51) |
| 알림톡·친구톡 | ADR 001: 사업자 인증·템플릿 심사 불가, 확장 로드맵 기재 |
| FCM 웹푸시 (S10) | 당시(구 work_breakdown01 §3 S10): P3, 마일스톤 후 |
| WebSocket | ADR 001 §2-1: 양방향 불필요, 기각 |
| 신청서 자동 제출 | CLAUDE.md 금지사항: 초안 생성까지만 |
| HWP 서버사이드 생성 | CLAUDE.md: 서버사이드 생성 불가, docx/PDF가 MVP 산출물 |
| Q9 국세청 상태조회 | 이후 실 연동 완료(이슈 #34) |
